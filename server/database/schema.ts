...
export const zhiluBudget = pgTable("zhilu_budget", {
  ...
  uniqueIndex("zhilu_budget_bucket_key").on(table.bucket),
]);

export const zhiluCatalog = pgTable("zhilu_catalog", {
  id: uuid("id").primaryKey().defaultRandom(),
  catalogKey: varchar("catalog_key", { length: 80 }).notNull().unique(),
  ...
  uniqueIndex("zhilu_catalog_catalog_key_key").on(table.catalogKey),
]);
